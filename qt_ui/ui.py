...
from PyQt6.QtWidgets import (
    QApplication, QComboBox, QFileDialog, QGridLayout, QHBoxLayout,
    QLabel, QLineEdit, QMainWindow, QPushButton, QSplitter,
    QTabWidget, QVBoxLayout, QWidget, QMessageBox
)

from qt_ui.status_panel_clean import StatusPanel

from qt_ui import tab_text_to_video
from qt_ui.tab_text_to_video import TextToVideoTab

from qt_ui import tab_image_to_video
from qt_ui.tab_image_to_video import ImageToVideoTab

from qt_ui import tab_character_sync
from qt_ui.tab_character_sync import CharacterSyncTab

from qt_ui import tab_create_image
from qt_ui.tab_create_image import CreateImageTab, CreateImageFromPromptTab

from qt_ui import tab_idea_to_video
from qt_ui.tab_idea_to_video import IdeaToVideoTab

from qt_ui import popup_theme
from qt_ui.popup_theme import install_messagebox_theme

from qt_ui import tab_settings
from qt_ui.tab_settings import SettingsTab

from qt_ui import tab_grok_settings
from qt_ui.tab_grok_settings import GrokSettingsTab

from qt_ui import branding_config
from qt_ui.branding_config import WINDOW_TITLE

from qt_ui import settings_manager
from qt_ui.settings_manager import BASE_DIR, get_icon_path
...
def apply_style(app: QApplication) -> None:
    ...
    QTabBar::tab:hover:!selected { background: #243a63; }
    ...
    QPushButton:pressed { background: #162945; }
    ...
    QPushButton#TopAction:hover { background: #1a4268; }
    ...
    QPushButton#DangerSoft:hover { background: #4c2432; }

    QPushButton#Accent { background: #2f6bff; border-color: #3f7cff; color: #ffffff; }
    QPushButton#Accent:hover { background: #2659d6; }
    QPushButton#Success { background: #1fa463; border-color: #35b87a; color: #ffffff; }
    QPushButton#Success:hover { background: #188353; }
    QPushButton#Warning { background: #f5be3c; border-color: #f3ce79; color: #1b2640; }
    QPushButton#Warning:hover { background: #dfa726; }
    QPushButton#Orange { background: #f07f2a; border-color: #f4a262; color: #ffffff; }
    QPushButton#Orange:hover { background: #d86f1f; }
    QPushButton#Danger { background: #e05252; border-color: #f07d7d; color: #ffffff; }
    QPushButton#Danger:hover { background: #ca4242; }
    ...
    QPushButton#Zalo:hover { background: #1a4268; }
    ...
    QComboBox#BottomCfgCombo {
        ...
        border-bottom: 1px solid #4a69a6;
        border-radius: 8px;
        padding: 4px 8px;
        padding-right: 24px;
        background: #1f3c70;
    ...
    QComboBox:hover { border-color: #4f6fa5; }
    QComboBox QAbstractItemView {
        background: #0f1d37;
        color: #e7eeff;
        border: 1px solid #2f4674;
        selection-background-color: #1f3f78;
        selection-color: #ffffff;
        outline: none;
    }
    ...
    QScrollBar:vertical { background: #0e1b34; width: 12px; margin: 0px; border-radius: 6px; }
    QScrollBar::handle:vertical { background: #334c79; border-radius: 6px; min-height: 30px; }
    QScrollBar::handle:vertical:hover { background: #4867a1; }
    QScrollBar::add-line:vertical, QScrollBar::sub-line:vertical { height: 0px; }
    QScrollBar::add-page:vertical, QScrollBar::sub-page:vertical { background: #0e1b34; }
    ...
    QWidget#HeaderBar {
        ...
        border-radius: 12px;
        background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 #132443, stop:1 #173057);
    }
    QLabel#HeroTitle {
        font-size: 16px;
        font-weight: 900;
...
class MainWindow(QMainWindow):
    def __init__(self, config: AppConfig, initial_security_report: client_security.SecurityReport | None = None):
        super().__init__()
        self._cfg = config
        ...
        header_bar_layout = QVBoxLayout(header_bar)
        header_bar_layout.setContentsMargins(12, 10, 12, 10)
        header_bar_layout.setSpacing(2)
        hero_title = QLabel('VEO/GROK Automation Studio')
        hero_title.setObjectName('HeroTitle')
        hero_subtitle = QLabel('Điều khiển workflow, theo dõi trạng thái, xuất video nhanh')
        hero_subtitle.setObjectName('HeroSubTitle')
        self.security_banner = QLabel('')
        self.security_banner.setObjectName('HeroSubTitle')
        self.security_banner.hide()
        header_bar_layout.addWidget(hero_title)
        header_bar_layout.addWidget(hero_subtitle)
        header_bar_layout.addWidget(self.security_banner)
        left_layout.addWidget(header_bar, 0)
        ...
        self.grok_tabs.setDocumentMode(True)
        ...
        bottom = QWidget()
        bottom.setObjectName('ControlDock')
        ...
        self._security_timer.setInterval(90000)
        self._security_timer.timeout.connect(self._security_tick)
        self._security_timer.start()
        ...
        def _set_initial_split():
            try:
                ...
                splitter.setSizes([int(total * 0.52), int(total * 0.48)])
            except Exception:
                pass
        ...
        self.status.setMinimumWidth(460)
        self.resize(1296, 805)

        if initial_security_report is not None:
            self._apply_security_report(initial_security_report, startup=True)
