...
import core.grok_chrome_manager as grok_chrome_manager
from core.grok_chrome_manager import kill_profile_chrome, open_profile_chrome, resolve_profile_dir
...
class GrokSettingsTab(QWidget):
    def __init__(self, config, parent: QWidget | None = None):
        ...
        lb_account_type.setStyleSheet('font-weight:700; color:#d7e3ff;')
        lb_length = QLabel('Thời gian video:')
        lb_length.setStyleSheet('font-weight:700; color:#d7e3ff;')
        lb_resolution = QLabel('Chất lượng video:')
        lb_resolution.setStyleSheet('font-weight:700; color:#d7e3ff;')
        lb_multi = QLabel('MULTI VIDEO:')
        lb_multi.setStyleSheet('font-weight:700; color:#d7e3ff;')
        ...
        note.setStyleSheet('color:#c3d7ff; font-size:12px; line-height:1.45; background:#0f1d37; border:1px solid #2f4674; border-radius:8px; padding:10px;')
        body.addWidget(note)
    ...
    def _show_profile_popup(self, profile_dir: str, host: str, port: int) -> None:
        ...
        title.setStyleSheet('font-size:16px; font-weight:800; color:#dce6ff;')
        layout.addWidget(title)
        
        info = QLabel()
        info.setWordWrap(True)
        info.setStyleSheet('color:#b8cdfb; font-size:12px;')
        layout.addWidget(info)
        
        status = QLabel('')
        status.setWordWrap(True)
        status.setStyleSheet('color:#e7eeff; font-size:12px; background:#0f1d37; border:1px solid #2f4674; border-radius:8px; padding:8px;')
