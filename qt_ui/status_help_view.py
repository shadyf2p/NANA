...
def _add_box(layout: QVBoxLayout, title: str, lines: list[str]) -> None:
    box = QWidget()
    box.setStyleSheet(
        'border:1px solid #2f4674; border-radius:12px; '
        'background:#0b1830;'
    )
    bl = QVBoxLayout(box)
    ...
    t = QLabel(title)
    t.setStyleSheet(
        'font-weight:900; color:#f3f7ff; font-size:13px; '
        'padding:6px 10px; border:1px solid #355489; border-radius:9px; '
        'background:#11284c;'
    )
    bl.addWidget(t)
    ...
    for ln in lines:
        ...
        lb.setWordWrap(True)
        lb.setStyleSheet(
            'border:1px solid #2f4674; border-radius:8px; background:#0f1d37; '
            'padding:6px 8px; color:#e7eeff;'
        )
        bl.addWidget(lb)
...
def build_status_help_view() -> QWidget:
    ...
    title.setStyleSheet('font-weight:800; color:#d7e3ff; font-size:14px;')
